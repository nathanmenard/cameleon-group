...
export default function ClientNotePage() {
  ...
  const getUser = useCallback(() => {
    ...
    const stored = localStorage.getItem("feedback_user");
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/comments?document_id=${DOCUMENT_ID}&token=${token}`
      );
  ...
  useEffect(() => {
    const handleTextSelection = (e: MouseEvent | TouchEvent) => {
      ...
      if (target.closest(".comments-sidebar") || target.closest(".selection-popup")) {
        return;
      }
      ...
      if (text.length > 3 && text.length < 500) {
        ...
        const section = range?.startContainer.parentElement?.closest("section[id]");

        if (section) {
          setCurrentSelection({ text, sectionId: section.id });
    ...
    try {
      await fetch(`${API_URL}/api/comments`, {
        ...
        body: JSON.stringify({
          ...
          section_id: currentSelection?.sectionId || "general",
  ...
  const handleResolveComment = async (id: number) => {
    ...
    try {
      await fetch(`${API_URL}/api/comments/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ author_token: user.token, is_resolved: true }),
      });
      await loadComments();
    } catch (error) {
      console.error("Error:", error);
  ...
  const handleDeleteComment = async (id: number) => {
    ...
    try {
      await fetch(`${API_URL}/api/comments/${id}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ author_token: user.token }),
      });
      await loadComments();
    } catch (error) {
      console.error("Error:", error);
  ...
  const scrollToText = (sectionId: string, selectedText?: string) => {
    ...
    while ((node = walker.nextNode()) && !found) {
      ...
      const index = textNode.textContent?.indexOf(selectedText.substring(0, 50)) ?? -1;
      if (index !== -1) {
        const range = document.createRange();
        range.setStart(textNode, index);
        range.setEnd(textNode, Math.min(index + selectedText.length, textNode.textContent?.length || 0));
  ...
  return (
    <>
      ...
      {/* Comments Sidebar */}
      <div className={`sidebar-backdrop ${sidebarOpen ? "" : ""}`} onClick={() => setSidebarOpen(false)} />
      <button className="sidebar-toggle" onClick={() => setSidebarOpen(!sidebarOpen)} title="Voir les commentaires">
        <span className="toggle-icon">💬</span>
        <span className="toggle-label">Commentaires</span>
        {unresolvedCount > 0 && <span className="badge">{unresolvedCount}</span>}
      </button>

      <div className={`comments-sidebar ${sidebarOpen ? "open" : ""}`}>
        <div className="sidebar-header">
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            <span className="sidebar-title">Commentaires</span>
          ...
          {comments.length === 0 ? (
            <div className="sidebar-empty">
              ...
              <div className="sidebar-empty-text">
                Sélectionnez du texte dans le document pour ajouter un commentaire
              </div>
            </div>
          ) : (
            comments.map((comment) => (
              <div
                key={comment.id}
                className={`comment-item ${comment.is_resolved ? "resolved" : ""} ${activeCommentId === comment.id ? "active" : ""}`}
                onClick={() => {
              ...
              >
                <div className="comment-header">
                  ...
                  {comment.is_internal && <span className="internal-badge">Interne</span>}
                </div>
                {comment.selected_text && (
                  <div className="comment-quote">&quot;{comment.selected_text.substring(0, 100)}{comment.selected_text.length > 100 ? "..." : ""}&quot;</div>
                )}
                <div className="comment-text">{comment.content}</div>
                {comment.is_owner && !comment.is_resolved && (
                  <div className="comment-actions">
                    <button className="action-btn" onClick={(e) => { e.stopPropagation(); handleResolveComment(comment.id); }}>✓ Résoudre</button>
                    <button className="action-btn delete" onClick={(e) => { e.stopPropagation(); handleDeleteComment(comment.id); }}>Supprimer</button>
                  </div>
                )}
                  ...
                  <div className="replies">
                    {comment.replies.map((reply) => (
                      <div key={reply.id} className="reply-item">
                        <div className="comment-header">
                          <div className="comment-avatar small">{reply.author_name.charAt(0).toUpperCase()}</div>
                          <div className="comment-meta">
                            <div className="comment-author">{reply.author_name}</div>
                            <div className="comment-time">{formatDate(reply.created_at)}</div>
                          </div>
                        </div>
                ...
                {!comment.is_resolved && (
                  replyingTo === comment.id ? (
                    <div className="reply-form-container">
                      <textarea className="reply-input" placeholder="Votre réponse..." id={`reply-${comment.id}`} />
                      <div className="reply-form-actions">
                        <button className="btn-cancel-sm" onClick={() => setReplyingTo(null)}>Annuler</button>
                        <button className="btn-submit-sm" onClick={() => {
                    ...
                    <button className="reply-btn" onClick={(e) => { e.stopPropagation(); setReplyingTo(comment.id); }}>Répondre</button>
                  )
                )}
              </div>
            ))
        ...
        {showNewCommentForm && (
          <form className="new-comment-form" onSubmit={handleSubmitComment}>
            {currentSelection?.text && (
              <div className="form-quote">&quot;{currentSelection.text.substring(0, 150)}{currentSelection.text.length > 150 ? "..." : ""}&quot;</div>
            )}
            {!user && (
              <div className="form-group">
                <label className="form-label">Votre nom</label>
                <input type="text" name="name" className="form-input" placeholder="Prénom Nom" />
              </div>
            )}
            <div className="form-group">
              <label className="form-label">Commentaire</label>
              <textarea name="content" className="form-textarea" placeholder="Écrivez votre commentaire..." />
            </div>
            <div className="form-group">
            ...
            <div className="form-actions">
              <button type="button" className="btn-cancel" onClick={() => setShowNewCommentForm(false)}>Annuler</button>
      ...
      {/* Selection Popup */}
      <div className="selection-popup" id="selectionPopup" style={{ display: currentSelection ? "flex" : "none" }}>
